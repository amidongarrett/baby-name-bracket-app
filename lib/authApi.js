 ...
 * @param {string} token
 * @param {string} [icon]
 * @returns {Promise<{ user: { id, email, displayName, icon } }>}
 */
export async function setName(displayName, token, icon) {
  const res = await fetch(`${BASE_URL}/api/auth/set-name`, {
    ...
    body: JSON.stringify({ displayName, ...(icon ? { icon } : {}) }),
  });
  const data = await res.json();
 ...
 * @param {string} token
 * @param {string} [icon]
 * @returns {Promise<{ user: { id, email, displayName, icon } } | { requiresVerification: true, message: string }>}
 */
export async function updateProfile(displayName, email, token, icon) {
  const body = {};
