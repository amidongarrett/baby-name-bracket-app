...
import { createBracket, getMyBrackets, joinBracket, joinBracketById } from '@/lib/lobbyApi';
...
function JoinModal({ token, isTestUser, allActive, guest, owned, onClose, onJoined }) {
  const [inviteCode, setInviteCode] = useState('');
  const [submitting, setSubmitting] = useState(false);
  const [joiningId, setJoiningId] = useState(null);
  const [error, setError] = useState(null);

  // For regular users: submit invite-code form
  const handleSubmit = async (e) => {
    e.preventDefault();
  ...
  // For test users: join by bracket ID directly
  const handleJoinById = async (bracketId) => {
    setError(null);
    setJoiningId(bracketId);
    try {
      const result = await joinBracketById(bracketId, token);
      onJoined(result.bracket);
    } catch (err) {
      setError(err.message);
    } finally {
  ...
  // Filter out brackets the user already owns or has joined
  const joinedOrOwnedIds = new Set([
    ...owned.map((b) => b._id || b.id),
    ...guest.map((b) => b._id || b.id),
  ]);
  const availableBrackets = (allActive || []).filter(
    (b) => !joinedOrOwnedIds.has(b._id || b.id)
  );
          ...
          <div className="flex flex-col gap-3">
            {availableBrackets.length === 0 ? (
              <p className="text-sm text-gray-500 dark:text-gray-400 py-4 text-center">
                No active brackets available to join.
              </p>
            ) : (
                  ...
                  return (
                    ...
                    >
                      <div className="flex flex-col gap-0.5 min-w-0 mr-3">
                        <span className="font-medium text-gray-900 dark:text-white text-sm truncate">{name}</span>
                        <StatusBadge status={bracket.status} />
                      </div>
                      <button
                        onClick={() => handleJoinById(id)}
                        disabled={isJoining}
                        className="shrink-0 px-3 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                      >
...
export default function LobbyPage() {
  ...
  const isTestUser = /^test\+.+@amidonlabs\.com$/i.test(user?.email ?? '');

  useEffect(() => {
    if (!token) return;
    setLoading(true);
    getMyBrackets(token)
      .then(({ owned: o, guest: g, allActive: a }) => {
        setOwned(o || []);
        setGuest(g || []);
        setAllActive(a || []);
        setError(null);
